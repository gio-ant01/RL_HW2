...
def generate_launch_description():
    ...
    # Imposta il path dei modelli Gazebo
    gazebo_model_path = PathJoinSubstitution(
        [FindPackageShare('iiwa_description'), 'gazebo', 'models']
    )
    
    if 'GZ_SIM_RESOURCE_PATH' in os.environ:
        os.environ['GZ_SIM_RESOURCE_PATH'] += ':' + str(gazebo_model_path)
    else:
        os.environ['GZ_SIM_RESOURCE_PATH'] = str(gazebo_model_path)
    ...
    # robot_description must be passed as a string parameter (not parsed as YAML)
    ...
    """ iiwa_simulation_world = PathJoinSubstitution(
        [FindPackageShare(description_package),
            'gazebo/worlds', 'newGazebo.world']
    )

    ##############################
    # Modifica l'argomento gz_args
    declared_arguments.append(
        DeclareLaunchArgument(
            'gz_args',
    ...
    # Ottieni il path assoluto del world file
    ...
    if not os.path.exists(iiwa_simulation_world):
        raise FileNotFoundError(f"World file not found: {iiwa_simulation_world}")

    declared_arguments.append(
        DeclareLaunchArgument(
            'gz_args',
            default_value=f'-r -v 1 {iiwa_simulation_world}',
            description='Arguments for gz_sim'
        )
    )
    ...
    #### 2c  Bridge per il servizio set_pose ##################
    pose_bridge = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        name='bridge_set_pose',
        arguments=[
